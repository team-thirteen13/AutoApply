...
export default function ProfileError({ error, reset }: ProfileErrorProps) {
  ...
  return (
    <div role="alert" className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50/30 to-violet-50/20 px-4 text-center">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">
          Something went wrong
        </h1>
        <p className="mt-3 text-sm text-slate-500">
          An unexpected error occurred while loading your profile. Please try
          again.
        </p>
        <button
          onClick={reset}
