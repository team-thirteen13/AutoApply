...
describe("getExperience", () => {
  ...
  it("rejects invalid IDs before auth and database access", async () => {
    ...
    for (const id of invalidIds) {
      ...
      const result = await getExperience(id as string);
  ...
  it("returns authentication_required when not authenticated", async () => {
    ...
    const result = await getExperience(VALID_UUID);
  ...
  it("builds correct query chain", async () => {
    ...
    await getExperience(VALID_UUID);
  ...
  it("returns mapped experience with snake_case to camelCase conversion", async () => {
  ...
  it("returns experience_not_found when data is null", async () => {
    mockMaybeSingle.mockResolvedValue({ data: null, error: null });
