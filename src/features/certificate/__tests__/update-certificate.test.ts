...
describe("updateCertificate", () => {
  ...
  it("rejects invalid ID before auth and database access", async () => {
    const result = await updateCertificate("bad-id", { name: "New" });
  ...
  it("rejects empty update payload", async () => {
    const result = await updateCertificate(VALID_UUID, {});
  ...
  it("returns authentication_required when not authenticated", async () => {
    ...
    const result = await updateCertificate(VALID_UUID, { name: "New" });
  ...
  it("builds correct update query and returns mapped certificate", async () => {
    ...
    if (result.success) {
      expect(result.data.name).toBe("AWS Solutions Architect Associate");
    }
  });
  ...
  it("sends only changed fields in update", async () => {
    ...
    await updateCertificate(VALID_UUID, { endDate: null });

    expect(mockUpdate).toHaveBeenCalledWith({ end_date: null });
  });

  it("returns certificate_not_found when data is null", async () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.code).toBe("certificate_not_found");
    }
  });
